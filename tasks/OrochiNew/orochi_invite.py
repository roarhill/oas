...
class OrochiFriendList(str, Enum):
...
class OrochiInvite(BaseTask, GeneralInviteAssets, OrochiNewAssets):
    timer_invite = None
    timer_wait = None
    ...
    def run_invite(self, config: InviteConfig, is_first: bool = False) -> bool:
        ...
        while 1:
            ...
            if self.room_type == OrochiRoomType.NORMAL_2 and not self.appear(self.I_ADD_2):
                logger.info('Start challenge and this room can only invite one friend')
                fire = True
            # 如果这个房间最多容纳三个人（意思是可以邀请两个人），且设定邀请一个就开启挑战，那就开启挑战
            elif self.room_type == OrochiRoomType.NORMAL_3 and config.invite_number == InviteNumber.ONE and not self.appear(self.I_ADD_1):
                logger.info('Start challenge and user only invite one friend')
                fire = True
            # 如果这个房间最多容纳三个人（意思是可以邀请两个人），且设定邀请两个就开启挑战，那就开启挑战
            elif self.room_type == OrochiRoomType.NORMAL_3 \
            ...
            elif self.room_type == OrochiRoomType.NORMAL_5 \
            ...
            # 如果这个房间是五人的，且设定邀请两个就开启挑战，那就开启挑战
                ...
                fire = True
            # 如果是永生之海
            elif self.room_type == OrochiRoomType.ETERNITY_SEA and not self.appear(self.I_ADD_SEA):
    ...
    def ensure_enter(self) -> bool:
        ...
        :return:
        """
        logger.info('Ensure enter orochi invite page')
        while 1:
            self.screenshot()
    ...
    def click_fire(self):
        while 1:
            ...
            if self.appear_then_click(self.I_FIRE_SEA, interval=1, threshold=0.7):
                continue
    @cached_property
    def room_type(self) -> OrochiRoomType:
        """
    ...
    def check_room_type(self, image: np.array = None, pre_type: OrochiRoomType = None) -> OrochiRoomType:
        """
        检查房间类型
        :param image:
        ...
        if pre_type is not None:
            match pre_type:
                case OrochiRoomType.NORMAL_2:
                    room_type = OrochiRoomType.NORMAL_2 if check_2(image) else None
                case OrochiRoomType.NORMAL_3:
                    room_type = OrochiRoomType.NORMAL_3 if check_3(image) else None
                case OrochiRoomType.NORMAL_5:
                    room_type = OrochiRoomType.NORMAL_5 if check_5(image) else None
                case OrochiRoomType.ETERNITY_SEA:
                    room_type = OrochiRoomType.ETERNITY_SEA if check_eternity_sea(image) else None
        if room_type:
            return room_type
        if room_type is None and check_2(image):
            room_type = OrochiRoomType.NORMAL_2
            return room_type
        if room_type is None and check_3(image):
            room_type = OrochiRoomType.NORMAL_3
            return room_type
        if room_type is None and check_5(image):
            room_type = OrochiRoomType.NORMAL_5
            return room_type
        if room_type is None and check_eternity_sea(image):
            room_type = OrochiRoomType.ETERNITY_SEA
            return room_type
        return room_type
    ...
    def ensure_room_type(self, friend_number: int = None) -> bool:
        ...
        if friend_number == 2:
            if self.room_type == OrochiRoomType.NORMAL_2:
                # 整个房间就可以两个人，还邀请两个 这个是报错的
                logger.error('Room can only be one people, but invite two people')
                return False
            elif self.room_type == OrochiRoomType.ETERNITY_SEA:
                # 永生之海，只能邀请一个人
                logger.error('Room can only be one people, but invite two people')
                return False
            return True
